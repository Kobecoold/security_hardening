...
class LinuxRollbackManager:
    ...
    def create_backup(
        ...
        try:
            ...
            try:
                # 1. Backup SSH Configuration (/etc/ssh/sshd_config)
                try:
                    ...
                    """
                    result = run_bash_check_stdin(ssh, backup_script, use_sudo=False)
                    
                    if result["exit_status"] == 0 and result["stdout"]:
                        backup_data["data"]["sshd_config"] = result["stdout"]
                        print(f"   ✓ SSH config backed up ({len(result['stdout'])} bytes)")
                    else:
                        # Thử với sudo nếu cần
                        result_sudo = run_bash_check_stdin(
                            ssh, backup_script, use_sudo=True, sudo_password=sudo_password
                        )
                        if result_sudo["exit_status"] == 0 and result_sudo["stdout"]:
                            backup_data["data"]["sshd_config"] = result_sudo["stdout"]
                            print(f"   ✓ SSH config backed up with sudo ({len(result_sudo['stdout'])} bytes)")
                except Exception as e:
                    print(f"   ⚠️ Failed to backup SSH config: {e}")
                
                # 2. Backup các file cấu hình quan trọng khác
                important_files = [
                    "/etc/passwd",
                    "/etc/shadow",
                    "/etc/group",
                ...
                backup_data["data"]["backup_info"] = {
                    ...
                    "host": host,
                    "username": username,
                    "notes": "Backup created before Linux remediation"
                }
