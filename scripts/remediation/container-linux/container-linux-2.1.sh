...
# Build docker run command KHÔNG có --privileged
NEW_CONTAINER_NAME="${CONTAINER_NAME}_fixed_$(date +%s)"

echo "📋 Creating new container WITHOUT privileged mode..."

DOCKER_CMD="docker run -d --name $NEW_CONTAINER_NAME"
# KHÔNG thêm --privileged
...
echo "✅ New container $NEW_CONTAINER_NAME created without privileged mode"
echo "⚠️  Please stop old container and rename new one:"
echo "   docker stop $CONTAINER_NAME"
echo "   docker rm $CONTAINER_NAME"
echo "   docker rename $NEW_CONTAINER_NAME $CONTAINER_NAME"
echo ""
echo "✅ Remediation completed - new container created without privileged mode"
exit 0
