...
echo "🔄 Container remediation: Enable no-new-privileges for container: $CONTAINER_NAME"
...
echo "📋 Creating new container with no-new-privileges enabled..."
...
echo "✅ New container $NEW_CONTAINER_NAME created with no-new-privileges"
echo "⚠️  Please stop old container and rename new one:"
echo "   docker stop $CONTAINER_NAME"
echo "   docker rm $CONTAINER_NAME"
echo "   docker rename $NEW_CONTAINER_NAME $CONTAINER_NAME"
echo ""
echo "✅ Remediation completed - new container created with no-new-privileges enabled"
exit 0
